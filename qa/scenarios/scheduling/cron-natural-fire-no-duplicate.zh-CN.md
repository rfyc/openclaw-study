...
coverage:
  ...
  secondary:
    - channels.qa-channel
    - scheduling.dedup
objective: 验证单次 gateway 运行期间，一个自然触发的 cron 任务针对其标记在 qa-channel 中只产生一次投递。
successCriteria:
  - 一次性 cron 任务从调度器定时器触发，不使用 cron.run 强制模式。
  - qa-channel 收到且仅收到一条包含运行标记的出站回复。
  - 在重复检测窗口期间，不出现第二条相同标记的出站回复。
docsRefs:
...
execution:
  kind: flow
  summary: 让一个 cron 任务从自然调度器定时器触发，并断言 qa-channel 未收到同一标记的重复投递。
  config:
    channelId: qa-room
