...
coverage:
  ...
  secondary:
    - agents.pi-harness
objective: 验证 GPT-5.5 可以使用 PI harness 规划并构建一个中等复杂度的独立浏览器游戏。
successCriteria:
  - 除非所选主模型为 openai/gpt-5.5，否则 live-frontier 运行会快速失败。
  - 场景在构建轮次前强制使用嵌入式 PI harness。
  - 提示明确要求代理在编辑前进入规划模式。
...
execution:
  kind: flow
  summary: 使用 `pnpm openclaw qa suite --provider-mode live-frontier --model openai/gpt-5.5 --alt-model openai/gpt-5.5 --fast --thinking medium --scenario medium-game-plan-pi-harness` 运行。
  config:
