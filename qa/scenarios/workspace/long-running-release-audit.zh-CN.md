...
coverage:
  ...
  secondary:
    ...
    - workspace.artifacts
objective: 验证模型能够完成持续性的代码库审计，该审计需要读取多个分散的文件、忽略过时的诱饵、写入工件并自查输出。
successCriteria:
  - 代理从种子项目 README 开始，而不是凭空猜测。
  - 代理跨多个目录检查文档和源文件。
  - 代理写入包含所有必要发现的 JSON 审计报告和 Markdown 交接文档。
  - 代理在最终确认前验证生成的 JSON。
docsRefs:
...
execution:
  kind: flow
  summary: 验证带有分散证据和工件验证的持续性代码库风格审计。
  config:
